...
class ProgramSerializer(serializers.ModelSerializer):
    ...
    facility_address = serializers.CharField(source='facility.fclty_addr', read_only=True)
    region = serializers.CharField(source='facility.sido.ctprvn_nm', read_only=True)
    days_display = serializers.CharField(source='get_days_display_korean', read_only=True)
    ...
    def get_region_image(self, obj):
        ...
        if obj.facility.sido.image and obj.facility.sido.image.url:
            url = obj.facility.sido.image.url
            if request:
